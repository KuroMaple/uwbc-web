...
export const playersSlice = createSlice({
  ...
  reducers: {
    ...
    movePlayer: (state, action: PayloadAction<PlayerDropAction>) => {
      //Find ID match
      const changedPlayer = state.players.find(
        (player) => player.id === action.payload.movedPlayerId,
...
export const selectPlayersinBench = createSelector(
  [selectPlayers],
  (players) => {
    return players.filter(
      (player: IPlayer) => player.position === Positions.Bench,
    );
  },
);
